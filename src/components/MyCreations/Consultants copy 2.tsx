// Enhanced AXEL Page with Improved UI/UX Design
"use client";
...
export default function ConsultantsPage() {
  ...
  const [processingConsultant, setProcessingConsultant] = useState<string | null>(null);
  ...
  const getCurrentConsultants = () => {
    if (consultantType === "saas") {
      return { axelConsultant: axelConsultant_saas, consultants: consultants_saas };
    } else if (consultantType === "business-automation") {
      return { axelConsultant: axelConsultant_businessAutomation, consultants: consultants_businessAutomation };
    } else {
      return { axelConsultant: axelConsultant_default, consultants: consultants_default };
    }
  };
  ...
  useEffect(() => {
    const getQuestionType = () => {
      ...
      else if (consultantType === "business-automation") return "businessAutomation";
      else if (consultantType === "saas") return "saas";
    };

    const questionType = sampleConsultantQuestions.find((q) => q.type === getQuestionType());
    setSampleQuestion(questionType?.question || "");
  }, [consultantType]);

  // Function to show call overlay with realistic messaging
  const showCallOverlay = (consultantName: string, phase: "calling" | "connected" | "responding" = "calling") => {
    const messages = {
      calling: [`📞 Calling ${consultantName}...`, `🔄 Connecting to ${consultantName}...`, `⏳ Waiting for ${consultantName} to pick up...`],
      connected: [`✅ ${consultantName} is on the line`, `🎙️ ${consultantName} is thinking...`, `🧠 ${consultantName} is analyzing your question...`],
      responding: [`💭 ${consultantName} is formulating response...`, `🎯 ${consultantName} has insights for you...`, `⚡ ${consultantName} is ready to respond...`],
    };

    const phaseMessages = messages[phase];
    const randomMessage = phaseMessages[Math.floor(Math.random() * phaseMessages.length)];

    setCallOverlay({ consultant: consultantName, message: randomMessage, phase: phase });
  };
  ...
  // Enhanced audio playback with video sync
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const dynamicUserPrompt = axelConsultant.userPrompt.replace("{{USER_QUESTION}}", inputText);
      ...
      if (conversationData.audioBase64) {
        try {
          ...
          const audioBytes = Uint8Array.from(atob(conversationData.audioBase64), (c) => c.charCodeAt(0));
          const audioBlob = new Blob([audioBytes], { type: conversationData.mimeType || "audio/mpeg" });
  ...
  const handleConsultantClick = async (consultant: Consultant): Promise<void> => {
    if (!conversation || isSpeaking) return;

    if (consultant.id === "anthony-hopkins") {
      if (conversation.consultantResponses.length < 2) {
        alert("Need at least 2 consultant responses before requesting synthesis from Anthony Hopkins.");
        return;
      }
    ...
    try {
      ...
      const response = await fetch("/api/consultants", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          type: consultant.isSpecial ? "hopkins_synthesis" : "consultant_question",
      ...
      if (consultantData.audioBase64) {
        try {
          ...
          const audioBytes = Uint8Array.from(atob(consultantData.audioBase64), (c) => c.charCodeAt(0));
          const audioBlob = new Blob([audioBytes], { type: consultantData.mimeType || "audio/mpeg" });
          consultantAudioUrl = URL.createObjectURL(audioBlob);
          playAudioWithVideo(consultantAudioUrl, consultant.id);
        } catch (error) {
          console.error(`Error converting ${consultant.name} base64 to audio:`, error);
        }
      }
        ...
        prev
          ? {
              ...
              consultantResponses: [
                ...
                {
                  consultant: consultant.isSpecial ? `${consultant.name} - Master Synthesis` : consultant.name,
                  response: consultantData.text || consultantData.response || "",
                  timestamp: new Date(),
      ...
      hideCallOverlay();
      changeVideoForSpeaker("axel");
      alert(`Sorry, there was an error consulting with ${consultant.name}. Please try again.`);
    } finally {
  ...
  const handleHopkinsSynthesis = async () => {
    if (!conversation || conversation.consultantResponses.length < 2 || isSpeaking) return;
    ...
    try {
      ...
      const consultationSummary = {
        ...
        consultantResponses: conversation.consultantResponses.map((resp, index) => ({
          order: index + 1,
...
${conversation.consultantResponses.map((resp) => `${resp.consultant}: "${resp.response}"`).join("\n")}
...
Remember: You must be ruthlessly concise. 80-120 words MAXIMUM. Count as you go. Stop at 120 words even if incomplete.`,
      ...
      if (hopkinsData.audioBase64) {
        try {
          ...
          const audioBytes = Uint8Array.from(atob(hopkinsData.audioBase64), (c) => c.charCodeAt(0));
          const audioBlob = new Blob([audioBytes], { type: hopkinsData.mimeType || "audio/mpeg" });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">AI Business Advisory Platform</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              {/* Current Speaker */}
              <div className="px-3 py-1 rounded-full bg-gray-800/50 border border-gray-700">
                <span className="text-xs text-gray-300">
                  {currentSpeaker === "axel" 
                    ? "AXEL" 
                    : consultants.find((c) => c.id === currentSpeaker)?.name || "AXEL"
                  }
                </span>
              </div>
            </div>
          ...
          <div className="xl:col-span-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-gradient-to-br from-gray-900/80 to-gray-800/80 rounded-2xl border border-gray-700/50 backdrop-blur-sm overflow-hidden">
                  <div className="p-6 border-b border-gray-700/50">
                    <div className="text-center">
                      ...
                      <p className="text-lg text-gray-300 font-semibold">{axelConsultant.title}</p>
                      <p className="text-sm text-gray-400 mt-1">{axelConsultant.expertise}</p>
                    </div>
                  </div>

                  {/* Video Container */}
                  <div className="relative">
                    <div className="aspect-[9/16] max-h-[500px] mx-auto relative overflow-hidden">
                      <video
                        ref={videoRef}
                      ...
                      {callOverlay && (
                        <div className="absolute inset-0 bg-black/80 flex items-center justify-center backdrop-blur-sm">
                          <div className="bg-gray-900/95 border-2 border-red-500/50 rounded-2xl p-8 text-center max-w-sm mx-4 shadow-2xl">
                            ...
                            <h3 className="text-white font-bold text-xl mb-3">{callOverlay.consultant}</h3>
                            <p className="text-gray-300 text-sm mb-4">{callOverlay.message}</p>

                            <div className="flex items-center justify-center space-x-3 text-xs text-gray-400">
                              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                              <span>
                                {callOverlay.phase === "calling" && "Establishing connection..."}
                                {callOverlay.phase === "connected" && "Line secured"}
                                {callOverlay.phase === "responding" && "Generating response..."}
                              </span>
                            </div>
                          </div>
                        </div>
              ...
              <div className="order-2 space-y-6">
                ...
                <div className="bg-gradient-to-br from-amber-900/20 to-orange-900/20 rounded-xl border border-amber-500/30 p-4">
                  <h3 className="text-lg font-semibold text-amber-300 mb-4">Choose Specialization</h3>
                  <div className="grid grid-cols-1 gap-3">
                    {[
                      { key: "default", label: "General Business", icon: "💼" },
                      { key: "business-automation", label: "Business Automation", icon: "🤖" },
                      { key: "saas", label: "SaaS Experts", icon: "💻" }
                    ].map(spec => (
                      <Button
                        key={spec.key}
                        className={`w-full justify-start space-x-2 transition-all duration-200 ${
                          consultantType === spec.key
                            ? "bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white"
                            : "bg-gray-700/50 hover:bg-gray-600/50 text-gray-300"
                        }`}
                        onClick={() => setConsultantType(spec.key)}
                      >
                        <span>{spec.icon}</span>
                ...
                <div className="bg-gradient-to-br from-gray-900/80 to-gray-800/80 rounded-xl border border-gray-700/50 p-6">
                  <h3 className="text-lg font-semibold text-gray-200 mb-4">Ask Your Question</h3>
                  ...
                  <textarea
                    ...
                    placeholder={sampleQuestion || "Ask AXEL anything about business, automation, or making money..."}
                  />
            ...
            {conversation && (
              <div className="mt-8">
                <div className="bg-gradient-to-br from-gray-900/80 to-gray-800/80 rounded-xl border border-gray-700/50">
                  <div className="p-6 border-b border-gray-700/50">
                    <div className="flex items-center justify-between">
                      <h3 className="text-xl font-bold text-red-400">💬 Consultation History</h3>
                      {conversation && (
                        <button
                          onClick={() => {
                            setConversation(null);
                  ...
                  <ScrollArea className="h-[500px] p-6">
                    ...
                    <div className="mb-6 p-4 bg-gradient-to-r from-blue-900/20 to-blue-800/20 border border-blue-500/30 rounded-lg">
                      <p className="text-blue-300 text-sm font-semibold mb-2">YOUR QUESTION:</p>
                      <p className="text-white leading-relaxed">{conversation.question}</p>
                    </div>
                    ...
                    <div className="mb-6 p-4 bg-gradient-to-r from-red-900/20 to-red-800/20 border border-red-500/30 rounded-lg">
                      ...
                      <p className="text-gray-200 leading-relaxed">{conversation.axelResponse}</p>
                    </div>

                    {/* Consultant Responses */}
                    {conversation.consultantResponses.map((response, index) => {
                      const isHopkinsSynthesis = response.consultant.includes("Hopkins") || response.isSpecial;
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex items-center space-x-2">
                              <span className="text-xl">{isHopkinsSynthesis ? "🎭" : "🧠"}</span>
                              <p className={`font-bold ${isHopkinsSynthesis ? "text-purple-300" : "text-emerald-300"}`}>
                          ...
                          <div className={isHopkinsSynthesis ? "text-purple-100" : "text-gray-200"}>
                            {isHopkinsSynthesis && (
                              <div className="mb-4 p-3 bg-purple-900/20 rounded-lg text-center">
                                <p className="text-xs text-purple-300 italic">
                                  &quot;I have observed your consultation with considerable interest...     &quot;
                                </p>
                              </div>
                            )}
            ...
            <div className="bg-gradient-to-br from-gray-900/80 to-gray-800/80 rounded-2xl border border-gray-700/50 backdrop-blur-sm sticky top-32">
              <div className="p-6 border-b border-gray-700/50">
                <div className="text-center">
                  ...
                  <div className="mt-3 px-3 py-1 bg-red-900/20 border border-red-500/30 rounded-full inline-block">
                    <span className="text-xs font-semibold text-red-300 uppercase">{consultantType}</span>
                  </div>
                </div>
              </div>

              <ScrollArea className="h-[600px] w-96  ">
                <div className="flex flex-col items-center justify-center gap-3">
                  {consultants.map((consultant) => {
                    ...
                    const hasResponded = conversation?.consultantResponses.some(
                      (r) => r.consultant.toLowerCase().includes(consultant.name.toLowerCase())
                    );

                    const isHopkins = consultant.id === "anthony-hopkins";
                    const canUseHopkins = isHopkins ? (conversation?.consultantResponses.length || 0) >= 2 : true;
                    const hopkinsMessage = isHopkins && !canUseHopkins ? "Need 2+ consultants first" : "";

                    return (
                      <div
                        key={consultant.id}
                        className={`relative w-80 rounded-xl p-1 transition-all duration-300 cursor-pointer ${
                          consultant.gradient
                        } border-2 ${consultant.borderColor} ${
                          !showConsultants || (isHopkins && !canUseHopkins) || isSpeaking || isProcessing
                            ? "opacity-50 cursor-not-allowed"
                            : "hover:scale-[1.02] hover:shadow-xl"
                        } ${isProcessing ? "animate-pulse" : ""} ${
                          hasResponded ? "ring-2 ring-green-400/50" : ""
                      ...
                      >
                        <div className="flex items-start space-x-6">
                          {/* Avatar */}
                          <div className="relative">
                            <div className="w-16 h-16 rounded-full overflow-hidden border-3 border-white/20 bg-gray-700">
                              {isProcessing ? (
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-bold text-white text-lg truncate">{consultant.name}</h3>
                            <p className="text-sm text-gray-200 font-medium truncate">{consultant.title}</p>
                            <p className="text-xs text-gray-300 mt-1 line-clamp-2">{consultant.expertise}</p>
                          </div>
                        ...
                        <div className="mt-4">
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-white/70 bg-black/20 rounded-full px-3 py-1">
                              {!showConsultants
                                ? "Ask AXEL first"
                                : isSpeaking
                                ? "Speaking..."
                            ...
                            {hasResponded && !isHopkins && (
                              <span className="text-xs text-green-300 font-medium">
                                {conversation?.consultantResponses.filter((r) =>
                                  r.consultant.toLowerCase().includes(consultant.name.toLowerCase())
                                ).length}x consulted
                              </span>
                            )}
                          </div>
                        ...
                        {isHopkins && (
                          <div className="mt-3 p-2 bg-purple-900/20 rounded-lg border border-purple-500/30">
                            <p className="text-xs text-purple-200 italic text-center">
                              &quot;I shall dissect each perspective with surgical precision...&quot;
                            </p>
                            {canUseHopkins && conversation && (
                              <p className="text-xs text-purple-300 mt-1 font-semibold text-center">
                                Ready to synthesize {conversation.consultantResponses.length + 1} perspectives
                              </p>
                            )}
                          </div>
                        )}
              ...
              <div className="p-6 border-t border-gray-700/50">
                <div className="bg-gradient-to-r from-blue-900/20 to-indigo-900/20 rounded-lg p-4 border border-blue-500/30">
                  ...
                  {conversation && conversation.consultantResponses.length > 0 && conversation.consultantResponses.length < 2 && (
                    <div className="mt-4 p-3 bg-yellow-900/20 border border-yellow-500/30 rounded-lg">
                      <p className="text-yellow-300 text-xs text-center">
                        💡 Consult with 2+ legends to unlock Anthony Hopkins synthesis
                      </p>
                    </div>
                  )}
                </div>
